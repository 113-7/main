...
$created_at = $data['date']; // 取得公告日期
$title = $data['title']; // 取得公告標題
$content = $data['content']; // 取得公告內容

// 準備 SQL 查詢，使用預處理語句防止 SQL 注入
$sql = "INSERT INTO announcements (department_id, created_at, title, content) VALUES (?, ?, ?, ?)";

// 使用預處理語句
$stmt = $conn->prepare($sql);

// 綁定參數
$stmt->bind_param("isss", $department_id, $created_at, $title, $content);
